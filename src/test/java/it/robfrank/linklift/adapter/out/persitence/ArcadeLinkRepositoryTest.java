...
class ArcadeLinkRepositoryTest {
  ...
  @Container
  private static final GenericContainer arcadeDBContainer = new GenericContainer("arcadedata/arcadedb:25.2.1")
  ...
  @BeforeAll
  static void setup() {
    RemoteServer server = new RemoteServer(arcadeDBContainer.getHost(), arcadeDBContainer.getMappedPort(2480), "root", "playwithdata");

    if (!server.exists("linklift")) {
      server.create("linklift");
    }
    RemoteDatabase database = new RemoteDatabase(arcadeDBContainer.getHost(), arcadeDBContainer.getMappedPort(2480), "linklift", "root", "playwithdata");
