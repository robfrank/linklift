...
public class AuthenticationService implements AuthenticateUserUseCase, RefreshTokenUseCase {
  ...
  public AuthenticateUserUseCase.@NonNull AuthenticationResult authenticate(@NonNull AuthenticateUserCommand command) {
    ...
    // Generate tokens
    var tokens = generateTokens(updatedUser, command);
  ...
  public AuthenticateUserUseCase.@NonNull AuthenticationResult refreshToken(@NonNull RefreshTokenCommand command) {
    ...
    // Generate new tokens
    var tokens = generateTokens(user, createRefreshCommand(command, false));
  ...
  private AuthenticateUserCommand createRefreshCommand(RefreshTokenCommand refreshCommand, boolean rememberMe) {
    return new AuthenticateUserCommand(
      "", // Not needed for refresh
      "", // Not needed for refresh
      refreshCommand.ipAddress(),
      refreshCommand.userAgent(),
      rememberMe
    );
  }
