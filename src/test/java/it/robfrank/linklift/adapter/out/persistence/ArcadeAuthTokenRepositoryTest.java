...
class ArcadeAuthTokenRepositoryTest {
  ...
  void revokeUserTokensByType_shouldRevokeOnlyTokensOfSpecificType() {
    ...
    // Assert
    List<AuthToken> userTokens = authTokenRepository.findByUserId(userId);
    Optional<AuthToken> sessionTokenResult = userTokens.stream().filter(t -> t.tokenType() == AuthToken.TokenType.SESSION).findFirst();
    Optional<AuthToken> refreshTokenResult = userTokens.stream().filter(t -> t.tokenType() == AuthToken.TokenType.REFRESH).findFirst();
