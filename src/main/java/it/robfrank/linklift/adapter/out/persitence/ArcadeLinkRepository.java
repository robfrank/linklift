...
public class ArcadeLinkRepository {
  ...
  public Link saveLink(Link link) {
    database.transaction(() -> {
      database.command("sql", """
          ...
          """, link.id(), link.url(), link.title(), link.description(), link.extractedAt(), link.contentType());
    });
    return link;
  }

  public Optional<Link> findLinkByUrl(String url) {
    return database.query("SELECT FROM Link WHERE url = ?", url)
        .stream()
        .findFirst()
  ...
  public Optional<Link> findLinkById(String id) {
    return database.query("SELECT FROM Link WHERE id = ?", id)
        .stream()
        .findFirst()
        .flatMap(Result::getVertex)
        .map(linkMapper::mapToDomain)
